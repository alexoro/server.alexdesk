...
describe('DAL::serviceUserGetRegisterConfirmData', function () {
    ...
    it('Must not pass invalid confirmToken', function (doneTest) {
        ...
        mock.executeOnClearDb(function (doneExecute) {
            var fnStack = [
                function (cb) {
                    api.serviceUserGetRegisterConfirmData(argsBuilder({id: {}}), cb);
                },
                function (cb) {
                    api.serviceUserGetRegisterConfirmData(argsBuilder({id: null}), cb);
                },
                function (cb) {
                    api.serviceUserGetRegisterConfirmData(argsBuilder({id: 1}), cb);
                },
                function (cb) {
                    api.serviceUserGetRegisterConfirmData(argsBuilder({id: '0cec4d47-d9a1-4984-XXXX-10583b674123'}), cb);
                }
            ];
