...
describe('DAL::apps_getNumberOfUnreadMessages', function () {
    ...
    it('Must not pass invalid id', function (doneTest) {
        ...
        mock.executeOnClearDb(function (doneExecute) {
            var fnStack = [
                function (cb) {
                    api.apps_getNumberOfUnreadMessages(argsBuilder({appIds: {}}), invalidArgsCallbackEntry(cb));
                },
                function (cb) {
                    api.apps_getNumberOfUnreadMessages(argsBuilder({appIds: null}), invalidArgsCallbackEntry(cb));
                },
                function (cb) {
                    api.apps_getNumberOfUnreadMessages(argsBuilder({appIds: [-1]}), invalidArgsCallbackEntry(cb));
                },
                function (cb) {
                    api.apps_getNumberOfUnreadMessages(argsBuilder({appIds: ['-1']}), invalidArgsCallbackEntry(cb));
                },
    ...
    it('Must return 0 if some application is not exists', function (doneTest) {
        ...
        mock.executeOnClearDb(function (doneExecute) {
            ...
            api.apps_getNumberOfUnreadMessages(reqArgs, function (err, result) {
                if (err) {
